...
export default function NavBar({}: Props) {
  return (
    <nav className="flex flex-row items-center z-20 justify-between text-left text-xl px-6 md:px-10 py-7 bg-transparent text-white font-inter">
      {/* left side */}
      <Link href="home"><Image src={logo} alt="logo" className=" w-32 md:w-auto" /></Link>
      ...
      <div className="flex flex-row items-center justify-start md:gap-6 gap-3 text-base text-placeholder">
        ...
        <div className="hidden md:flex gap-4">
          <Link href="/sign-up"><Button variant={"gradiend"}>Sign Up</Button></Link>
          <Link href="/sign-in"><Button variant={"transparent"} className=" py-3 px-0 text-sm ">
            Sign In
          </Button>
          </Link>
        </div>
